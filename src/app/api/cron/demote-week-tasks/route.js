...
import { verifyCronAuth, getLondonDayOfWeek, claimCronRun, updateCronRun } from '@/lib/cronAuth';
...
export async function GET(request) {
  try {
    ...
    // FF-002 belt-and-braces: if the user has already completed the upcoming
    // weekly planning session, the plan is made — never demote it, even if a
    // mistimed or forced run reaches this point. On Sunday windowDate is tomorrow
    // (Monday), matching the client's getActivePlanningWindow recorded row.
