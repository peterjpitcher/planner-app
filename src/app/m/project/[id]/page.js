...
const getPriorityStyles = (priority) => {
  switch (priority) {
    case 'High': return { icon: <FireIcon className="h-5 w-5 text-red-500 inline mr-1" />, textClass: 'text-red-600 font-semibold' };
    case 'Medium': return { icon: <ExclamationTriangleIcon className="h-5 w-5 text-yellow-500 inline mr-1" />, textClass: 'text-yellow-600 font-semibold' };
    case 'Low': return { icon: <SolidCheckIcon className="h-5 w-5 text-green-500 inline mr-1" />, textClass: 'text-green-600' };
    default: return { icon: <ClockIcon className="h-5 w-5 text-gray-400 inline mr-1" />, textClass: 'text-gray-500' };
  }
};
  ...
  const handleTaskUpdatedFromSwipe = useCallback((updatedTask) => {
    setTasks(currentTasks => currentTasks.map(t => t.id === updatedTask.id ? updatedTask : t)
      .sort((a, b) => {
        ...
        if (dateA && dateB) { const dateComparison = compareAsc(dateA, dateB); if (dateComparison !== 0) return dateComparison; }
        else if (dateA) return -1; else if (dateB) return 1;
        return (getPriorityValue(b.priority) || 0) - (getPriorityValue(a.priority) || 0); // Ensure consistent sorting
      })
    );
  ...
  if (authLoading || isLoading) {
    return (
      <MobileLayout title="Loading Project...">
        <div className="text-center py-10"><p className="text-gray-500">Loading project details...</p></div>
      </MobileLayout>
    );
  }
    ...
    <MobileLayout title={project.name || 'Project Details&apos;'}>
      <div className="bg-white shadow-md rounded-lg p-4 mb-4">
        <div className="flex justify-between items-center mb-3 space-x-2">
          ...
          <button onClick={handleEditProject} className="p-2 rounded-full hover:bg-gray-100 flex-shrink-0" title="Edit Project">
        ...
        <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm mb-3">
          <div>
            ...
            <span className={`ml-2 px-2 py-0.5 rounded-full text-xs font-medium ${statusClass}`}>
              {project.status || 'N/A'}
            </span>
          </div>
          <div>
            ...
            <span className={`ml-2 inline-flex items-center ${priorityInfo.textClass}`}>
              {priorityInfo.icon}{project.priority || 'N/A'}
            </span>
          </div>
          <div>
            <span className="font-medium text-gray-500">Due Date:</span>
            <span className="ml-2 text-gray-700">
              {project.due_date ? format(parseISO(project.due_date), 'MMM d, yyyy') : 'N/A'}
            </span>
          </div>
          {project.stakeholders && project.stakeholders.length > 0 && (
            <div className="col-span-2">
              <span className="font-medium text-gray-500">Stakeholders:</span>
              <span className="ml-2 text-gray-700 break-words">{project.stakeholders.join(', ')}</span>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="bg-white shadow-md rounded-lg p-0">
        <div className="flex justify-between items-center p-3 border-b border-gray-200">
          <h2 className="text-lg font-semibold text-gray-700">Tasks ({tasks.filter(t => !t.is_completed).length} open)</h2>
          <button onClick={handleAddTask} className="p-1.5 rounded-full hover:bg-indigo-50" title="Add New Task">
        ...
        {tasks.length > 0 ? (
          ...
          <p className="text-sm text-gray-500 p-4 text-center">No tasks for this project yet.</p>
        )}
      </div>

      {/* Project Notes Section */}
      <div className="mt-4 bg-white shadow-md rounded-lg p-4">
        <h2 className="text-lg font-semibold text-gray-700 mb-2">Project Notes</h2>
