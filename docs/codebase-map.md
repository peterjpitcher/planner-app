...
Reference material moved out of `CLAUDE.md` on 2026-09-04 so that file holds only rules. Everything below was checked against the repo on that date; when the code moves, update this file. Related: `docs/architecture/` (generated overview, routes, data model) and `docs/superpowers/specs/` (one design spec per feature wave).
